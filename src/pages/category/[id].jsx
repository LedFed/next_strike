...
export async function getStaticPaths() {
    const categories = ['Дым', 'Гранаты', 'МинаРастяжка','Пневматика'];
...
const categoriesData = {
    ...
    'Петарды': [{ title: 'Петарды: Как они работают?', content: 'Петарды — небольшие пиротехнические изделия, которые при срабатывании быстро выделяют энергию в виде звука, огня и ударной волны. Их устройство и принцип действия основаны на ограниченном объёме горючей пиротехнической зарядки и способе воспламенения; даже «маленький» заряд представляет серьёзную опасность для рук и слуха.' }, { title: 'Насколько опасна петарда?', content: ' Радиус бризантного действия мощных петард — несколько сантиметров; при взрыве в руке часто наблюдаются тяжёлые повреждения конечностей и ампутации. Петарды 3 класса и выше опасности не рекомендуеться поджигать в руках' },
    { title: 'Что это такое и из каких частей состоят петарды?', content: 'Корпус: обычно бумажный или пластмассовый цилиндр/шар, который удерживает заряд внутри и частично ограничивает его расширение.Пиротехнический заряд: компактная смесь, которая при возгорании быстро выделяет газы и тепло (на практике это сочетание окислителя, горючего и других связующих. Запал/воспламенитель (фитиль): элемент, дающий время до срабатывания и управляющий моментом поджига. ' }]
    , 'Пневматика': [{ title: 'Нужна ли лицензия на покупку пневматики?', content: 'Все зависит от дульной энергии изделия. До 3 Дж: С точки зрения закона это «конструктивно сходное с оружием изделие». Лицензия не нужна, продается свободно лицам от 18 лет. От 3 до 7.5 Дж (калибр 4.5 мм): Это гражданское спортивное оружие. Лицензия не требуется, но покупка возможна только в специализированных магазинах при предъявлении паспорта. В нашем магазине предоставлены модели которые не требуют лицензию на ношения' },
    { title: 'Можно ли охотиться с пневматикой?', content: 'Охота разрешена только с использованием сертифицированного охотничьего пневматического оружия (дульная энергия до 25 Дж) при наличии охотничьего билета и разрешения. Охота с «развлекательной» пневматикой до 3 или 7.5 Дж считается браконьерством.' },
    { title: 'Можно ли хранить пневматику заряженной?', content: 'В пружинно-поршневых моделях проседает боевая пружина, что снижает мощность. В газобаллонных моделях (CO2) постоянное давление на прокладки приводит к их быстрому рассыханию и утечке газа. С точки зрения безопасности: оружие всегда должно храниться разряженным.' },
    { title: 'Можно ли возить пневматику в машине?', content: 'Да, транспортировка разрешена. Главное правило: изделие должно находиться в чехле или заводской упаковке в разряженном состоянии. Желательно иметь при себе паспорт изделия или сертификат (особенно для моделей до 3 Дж), чтобы у сотрудников ДПС не возникло лишних вопросов.' },
    { title: 'Можно ли носить пневматический пистолет с собой для самообороны?', content: 'Категорически нет. Пневматика не предназначена для самообороны. Во-первых, её останавливающее действие ничтожно (вы только разозлите нападающего). Во-вторых, закон запрещает использование пневматики в целях защиты. Ношение пневматики в заряженном виде в общественных местах может привести к конфискации и штрафу.' }]
};
...
const metaTeg = {
    ...
    'Дым': { title: 'Дымовые гранаты', chapter: 'Дымовые гранаты создают облако дыма, которое используется для маскировки.' },
    'МинаРастяжка': { title: 'Мины-растяжки', chapter: 'Мины-растяжки активируются при натяжении, создавая эффект неожиданности.' },
    'Петарды': { title: 'Петарды', chapter: 'Петарды взрывное пиротехническое изделие. Популярные развлекательные петарды разной мощности от самых мощных до обычных корсаров' },
    'Пневматика': { title: 'Пневматика', chapter: 'Узнайте, нужна ли лицензия на пневматику, где можно стрелять и можно ли использовать пневматический пистолет для охоты и самообороны.' },
};
